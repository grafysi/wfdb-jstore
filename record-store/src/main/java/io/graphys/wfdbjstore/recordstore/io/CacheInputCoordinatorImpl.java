...
import io.graphys.wfdbjstore.recordstore.exception.CacheWriterFailedException;
...
public class CacheInputCoordinatorImpl implements CacheInputCoordinator {
    private static final Logger logger = LogManager.getLogger(CacheInputCoordinatorImpl.class);
    ...
    public void prepareCache(boolean refresh) {
        writing.writeToCache(executorService, refresh);
    }

    @Override
    public void awaitPrepareCache(boolean refresh) {
        if (!writing.writeToCache(refresh)) {
            writing.writeLock.lock();
            try {
                if (writing.active.get()) {
                    writing.writingCompleted.await();
                }
            } catch (Exception e) {
    ...
    class Reading {
        ...
        private InputStream getReaderOf(InputBackend inputBackend) throws IOException {
            ...
            try {
                ...
                return switch (inputBackend) {
                    case SOURCE_ORIGINAL_STREAM -> source.openStream();
                    case CACHE_FILE_STREAM -> {
                        numReaders.getAndIncrement();
                        yield new CoordinatedInputStream(new FileInputStream(cacheFile), this);
                    }
                    case CACHE_FILE_RANDOM_ACCESS -> {
                        numReaders.getAndIncrement();
                        yield new CoordinatedRandomFileInputStream(cacheFile, this);
                    }
                    default -> throw new IllegalArgumentException("Unsupported input backend: " + inputBackend);
                };
    ...
    class Writing {
        ...
        // writing completed condition
        private final Condition writingCompleted = writeLock.newCondition();
        ...
        private void writeToCache(ExecutorService es, boolean refresh) {
            if (prepareWriting() && !refresh) {
                return;
            }
        ...
        private boolean writeToCache(boolean refresh) {
            if (prepareWriting() && !refresh) {
                return false;
            }
            writeToCacheHelper();
        ...
        private void writeToCacheHelper() {
            /*prepareWritingLock.lock();
            try {
                if (isActive() || (!hadPreviouslyFailed() && cacheFile.isFile() && cacheFile.canRead())) {
                    return false;
            ...
            try (var in = source.openStream();
                ...
                var conn = source.openConnection();
